...
void WorldSession::HandleRepopRequestOpcode(WorldPacket & recv_data)
{
    ...
    else if (_player->IsInDuelingZone())
    {
        ...
        _player->ResurrectPlayer(1.0f);
        _player->SpawnCorpseBones();
        _player->TeleportTo(0, 1976.65f, 2546.16f, 131.25f, 4.7f);
        return;
    }
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket & /*recv_data*/)
{
    ...
    if (_player->isInCombat())
        reason = 1;
    else if (_player->HasAura(9454)) // is dueling or frozen by GM via freeze command
    ...
    if (_player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) || _player->isInFlight() || (!_player->IsInZangarmarsh() || _player->GetSession()->GetSecurity() >= SEC_GAMEMASTER))
    {
        WorldPacket data(SMSG_LOGOUT_RESPONSE, 1+4);
        data << uint8(0);
