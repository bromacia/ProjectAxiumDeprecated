#include "OutdoorPvPZG.h"

OutdoorPvPZG::OutdoorPvPZG()
{
    m_TypeId = OUTDOOR_PVP_ZG;
    m_ally_gathered = ZG_RESOURCES_A;
    m_horde_gathered = ZG_RESOURCES_H;
...
bool OutdoorPvPZG::SetupOutdoorPvP()
{
    RegisterZone(3537);
    return true;
}

void OutdoorPvPZG::HandlePlayerEnterZone(Player* player, uint32 zone)
{    
    char message[250];
    if(player->GetTeam() == ALLIANCE)
        snprintf(message, 1024, "[Zangarmash Defense]: Alliance has %u resources!", m_ally_gathered);
    else
        snprintf(message, 1024, "[Zangarmash Defense]: Horde has %u resources!", m_horde_gathered);

    player->MonsterTextEmote(message, player->GetGUID());
    OutdoorPvP::HandlePlayerEnterZone(player, zone);
}

void OutdoorPvPZG::HandlePlayerLeaveZone(Player * player, uint32 zone)
{
    player->MonsterTextEmote("You're leaving while the zone PvP is active!", player->GetGUID());
    OutdoorPvP::HandlePlayerLeaveZone(player, zone);
}

void OutdoorPvPZG::HandleWinMessage(Player* player, uint8 type, const char * message)
{
    char str[250];
    va_list ap;
    va_start(ap, message);
...
void OutdoorPvPZG::PlaySounds(bool side)
{
...
void OutdoorPvPZG::HandleReset()
{
    ...
    limit_resources_message_A = 0;
    limit_resources_message_H = 0;
    sLog->outString("[OutdoorPvPZG]: Zangarmash : HandleReset()");
}

void OutdoorPvPZG::HandleBuffs(Player* player, bool loser)
{
    if(loser)
    {
...
void OutdoorPvPZG::HandleRewards(Player* player, uint32 honorpointsorarena, bool honor, bool arena, bool both)
{
    char msg[250];
    ...
    HandleWinMessage(player, CHAT_MSG_MONSTER_EMOTE, msg);
}

bool OutdoorPvPZG::Update(uint32 diff)
{
    OutdoorPvP::Update(diff);
    if(m_FirstLoad == false)
    {
        if(m_LastWin == ALLIANCE)
           sLog->outString("[OutdoorPvPZG]: Borean Tundra has started! Last Winner: Alliance(%u)", ALLIANCE);
        else if(m_LastWin == HORDE)
           sLog->outString("[OutdoorPvPZG]: Borean Tundra has started! Last Winner: Horde(%u)", HORDE);
        else if(m_LastWin == 0)
           sLog->outString("[OutdoorPvPZG]: Borean Tundra has started! Last Winner: Neutral(0)");
        m_FirstLoad = true;
    }
            ...
            if(itr->second->GetPlayer()->GetZoneId() == 3537)
            {
                ...
                if(IS_RESOURCE_MESSAGE_A == true)
                {
                    ...
                    else if(limit_A == 2)
                    {
                        ...
                        HandleWinMessage(itr->second->GetPlayer(), CHAT_MSG_RAID_WARNING, "Horde Wins!");
                ...
                else if(IS_RESOURCE_MESSAGE_H == true)
                {
                    ...
                    else if(limit_H == 2)
                    {
                        ...
                        itr->second->GetPlayer()->GetGUID());
                        HandleWinMessage(itr->second->GetPlayer(), CHAT_MSG_RAID_WARNING, "Alliance Wins!");
                        HandleRewards(itr->second->GetPlayer(), 1500, true, false, false);
                        switch(itr->second->GetPlayer()->GetTeam())
                        {
                           case ALLIANCE:
...
void OutdoorPvPZG::Randomizer(Player* player)
{
    switch(urand(0, 4))
    {
        case 0:
...
void OutdoorPvPZG::HandleKill(Player* player, Unit* killed)
{
